...
class Scene():
    ...
    Notes:
        ...
        4) Given a measurement (x,y), the
           associated bounding box is [x-width/2,x+width/2]x[y-width/2,y+width/2]
    ...
    def __init__(self, name, BB_ts_ls, width, height, k_max_alpha=6, k_max_theta=6):
        ...
        theta_coeffs = np.zeros((self.num_nl_classes,
                                 self.k_max_theta+1,
                                 self.k_max_theta+1))
        for k, cl in enumerate(clusters):
            theta_coeffs[k] = trajectories_to_director_field(cl, width, height,
        ...
        for i, curve in enumerate(curve_ls):
            ...
            for k in range(self.num_nl_classes):
                ode_forward = lambda x, t: speed * self.director_field(k, x) # pylint: disable=cell-var-from-loop
                ode_backward = lambda x, t: -speed * self.director_field(k, x) # pylint: disable=cell-var-from-loop
    ...
    def director_field(self, k, x, jac=False):
        ...
        theta = legval2d(x[0] / self.width,
                         x[1] / self.height,
                         self.theta_coeffs[k])
        out1 = np.array([np.cos(theta), np.sin(theta)])
        if jac:
            dtheta_dx = legval2d(x[0] / self.width,
                                 x[1] / self.height,
                                 legder(self.theta_coeffs[k], axis=0)) / self.width
            dtheta_dy = legval2d(x[0] / self.width,
                                 x[1] / self.height,
                                 legder(self.theta_coeffs[k], axis=1)) / self.height
            out2 = np.zeros((2, 2))
...
if __name__ == "__main__":
    ...
    test_scene = Scene("test_scene", train_set, width, height)
    ...
    response = input("Would you like to pickle this scene? y/n")
    if response == "y":
