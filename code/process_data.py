...
Module contains methods for reading annotation files and
processing bounding box trajectory lists from them.
"""
...
def get_BB_ts_list(folder):
    ...
    def enters_and_leaves(BB_ts):
        ...
        """
        outside = lambda left, bottom, right, top: (left < -0.9*width/2 or
                                                    right > 0.9*width/2 or
                                                    bottom < -0.9*height/2 or
                                                    top > 0.9*height/2)
...
def BB_to_position_and_velocity(BB_0, BB_1, delta_t=1):
    """
    Returns the position and velocity of a pedestrian given two time-adjacent bounding boxes.
    """
...
def get_std_velocity(BB_ts_ls):
    ...
    def BB_ts_to_std(BB_ts):
        """
        Computes standard deviation of velocity of a bounding box time series.
        """
...
if __name__ == '__main__':
    ...
    BB_ts_list, width, height = get_BB_ts_list("../annotations/coupa/video2/")
