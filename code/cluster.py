...
This module provides clustering functions for trajectories
based on the clustering given in our paper.

"""
from functools import reduce
import numpy as np

__consume = lambda x: [y for y in x]
...
def prune_cluster(cluster):
    ...
    def length_of_traj(traj):
        "Returns geometric length of traj"
        n = traj.shape[1]
...
def learn_potential(cluster, width, height, k_max=8, stride=30):
    """
    Returns the legendre coefficients of a potential function
    learned from a list of point in a 2D domain.
    ...
    returns:
        out (numpy.ndarray) : coeffs for 2D Legendre series describing potential
                              funciton. shape = (k_max+1, k_max+1)
    """
    ...
    def cost_function(theta_flat):
        """Cost function of optimization from the paper"""
    ...
    def potential_constraint(theta_flat):
        """constraint function for learining"""
        return theta_flat[0]
    constraint_list = []
    ...
    res = minimize(cost_function, initial_guess,
                   ...
                   options={'maxiter':cb_function.bar.max})
...
def get_classes(curves, width, height, k_max=4):
    ...
    NOTE: alpha[k] and clusters_pruned[k] have probabilitiy P_of_c[k].
          k=-1 corresponds to a linear predictor
    """
    clusters = cluster_trajectories(curves)

    #Prune the clusters and keep track of how many agents you discard
    n_discarded = 0
    for k, _ in enumerate(clusters):
